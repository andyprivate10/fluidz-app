...
export function formatElapsed(startRef: string): string {
  ...
  if (untilStart < 60) return 'dans ' + untilStart + 'min'
  return 'dans ' + Math.floor(untilStart / 60) + 'h'
}
...
export function formatMessageTime(dateStr: string): string {
  ...
  if (diffMs < 7 * 24 * 3600000) return d.toLocaleDateString('fr-FR', { weekday: 'short', hour: '2-digit', minute: '2-digit' })
