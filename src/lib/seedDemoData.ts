...
export async function seedDemoData(onProgress?: SeedProgress): Promise<void> {
  ...
  const s1Id = await upsertSession({
    ...
    template_slug: 'darkroom', tags: ['dark_room', 'intime', 'safe'],
  ...
  const s3Id = await upsertSession({
    ...
    template_slug: 'custom', tags: ['leather', 'fetish', 'dresscode'],
  ...
  const s4Id = await upsertSession({
    ...
    template_slug: 'custom', tags: ['spontané', 'chill', 'groupe'],
  ...
  const s5Id = await upsertSession({
    ...
    template_slug: 'custom', tags: ['weekend', 'chill'],
    max_capacity: 8,
    invite_code: 'WKEND1',
    lineup_json: {},
  })
