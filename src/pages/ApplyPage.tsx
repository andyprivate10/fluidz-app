  ...
  if (!d.user && !d.guestMode) return (
    <div style={{minHeight:'100vh',background:S.bg,display:'flex',alignItems:'center',justifyContent:'center'}}>
      <div style={{textAlign:'center',padding:24}}>
        ...
        <button onClick={() => d.navigate('/login?next=' + encodeURIComponent('/session/' + d.id + '/apply'))} style={{padding:'12px 24px',borderRadius:12,background:S.grad,color:'#fff',border:'none',fontWeight:700,cursor:'pointer'}}>Se connecter</button>
      </div>
    </div>
  )
