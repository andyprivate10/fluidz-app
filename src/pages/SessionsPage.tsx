...
export default function SessionsPage() {
  ...
  const card: React.CSSProperties = { background: S.bg1, border: `1px solid ${S.rule}`, borderRadius: R.card, padding: 16, cursor: 'pointer' }
                ...
                <div key={sess.id} onClick={() => navigate(isEnded ? '/session/' + sess.id : '/session/' + sess.id + '/host')} style={card}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                    ...
                    <span style={{
                      ...
                      background: isOpen ? S.sagebg : isEnded ? '#F8717114' : S.bg2,
                      border: `1px solid ${isOpen ? S.sagebd : isEnded ? '#F871712e' : S.rule}`,
